...
def load_backbone(args):
    ...
    elif args.backbone=='Tmamba':
        from lib.model.Tmamba import Tmamba
        model_backbone = Tmamba(dim_in=3, dim_out=3,dim_feat=args.dim_feat, dim_rep= args.dim_rep, 
                                 depth= args.depth, num_joints= args.num_joints, maxlen= args.maxlen,
                                 norm_layer=partial(nn.LayerNorm, eps=1e-6))
    elif args.backbone=='STmamba':
        from lib.model.STmamba import STmamba
        model_backbone = STmamba(dim_in=3, dim_out=3,dim_feat=args.dim_feat, dim_rep= args.dim_rep, 
                                 depth= args.depth, num_joints= args.num_joints, maxlen= args.maxlen,
                                 norm_layer=partial(nn.LayerNorm, eps=1e-6),
